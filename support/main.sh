...
__source_script_if_needed "build_project" "${_SCRIPT_DIR:-}/build.sh" || exit 1
...
__main() {
  ...
  case "${_command:-}" in
    ...
    build-docs|BUILD-DOCS|-bdc|-BDC)
      ...
      mkdocs build --no-strict -c -f "${_ROOT_DIR:-}/support/docs/mkdocs.yml" -d "${_ROOT_DIR:-}/docs-site" -q || {
